...
task(
  ...
  async (_, hre) => {
    ...
    if (chainId === '31337') {
      console.error(
        'Hardhat network is not valid for etherscan. Please use remote network.'
      );
      return;
    }
    const { host, networkName } = etherscanHost(chainId);
    const rsp = await submit(host, networkName);
    if (rsp?.status == 200 && rsp.data.status == 1) {
      console.log('Successfully verified proxy contracts on etherscan');
    }
  }
);
